...
ball:
	...
	R:
	cmp word[cs:colume],3
	jne nextcond  ;this is the edited one
	jmp printingLocation ; this is to be deleted
		mov byte[cs:incC],1
		jmp rowCheck
	nextcond:
		cmp word[cs:colume],77
	...
	printingLocation:
		...
		rowCheck1:
			cmp byte[cs:incR],1
			jne nextcond3
				;add word[cs:row],1   chnage it back
...
main_game:
	...
	nextcmp5:
		cmp al,0xb9
		mov byte[cs: start_game_here_a],1 ;edited
		jne exitcmp
		jmp exit
...
iret 
timer: 
	cmp byte[cs:start_game_here_a],1
	jne pp
		inc byte[cs:clock]
